...
contract OnchainNft {
  ...
  function _generateMetadata(
    ...
    string memory metadataDescription = string(abi.encodePacked(description));
  ...
  function _generateSvg(
    ...
    string memory svg = string(
      abi.encodePacked(
        '<svg width="400" height="400" xmlns="http://www.w3.org/2000/svg">', _renderToken(components), "</svg>"
      )
    );
  ...
  function generateMetadata(
    ...
    metadata = _generateMetadata(name, description, attributes, svgComponents);
    metadata =
      string(abi.encodePacked("data:application/json;base64,", Base64.encode(bytes(abi.encodePacked(metadata)))));
  }
}
