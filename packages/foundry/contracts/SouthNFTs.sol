...
contract SouthNFTs is ERC721A {
  ...
  function addTrait(uint256 layer, Payload calldata payload, uint16 rarity) public {
  ...
  // ********************************
  function _setTrait(uint256 traitIndex, uint256 layer, Payload calldata payload, uint16 rarity) internal {
...
uint256 attemptCount = 0;
    ...
    while (true) {
      ...
      combination |= (_getRandomTraitIndex(1, armorRarities, seed >> 16) << 8);
      combination |= (_getRandomTraitIndex(2, headRarities, seed >> 32) << 16);
      combination |= (_getRandomTraitIndex(3, weaponRarities, seed >> 48) << 24);
  ...
  function _storeTraits(uint256 tokenId, uint256 traitCombination) internal {
    ...
    _registry[bucket] = _calculateSlotInBucket(tokenId, traitCombination, bucket);
  }

  function _calculateSlotInBucket(uint256 key, uint256 value, uint256 bucket) internal view returns (uint256) {
    uint256 slot = key % 4;
  ...
  // ********************************
  function getTrait(uint256 layer, uint256 traitIndex) public view returns (Trait memory) {
  ...
  function _getTraitAttributes(
    ...
    return string.concat(
      _getTraitMetadata("BACKGROUND", s_traits[0][knight.background].name, false),
      ",",
      _getTraitMetadata("ARMOR", s_traits[1][knight.armor].name, false),
      _getTraitMetadata("HEAD", s_traits[2][knight.head].name, true),
      _getTraitMetadata("WEAPON", s_traits[3][knight.weapon].name, true)
    );
  }
  ...
  function _getTraitImageData(
    ...
    return string(abi.encodePacked("url(data:image/png;base64,", image, ")"));
  }
